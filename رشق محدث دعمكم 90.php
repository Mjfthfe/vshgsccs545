<?php
$Token = "7322176075:AAEj46TWIdmv2-orEmMIhhaZCK_Q986nYAU";//توكنك//
$admin = 1031017547;// ايديك //
...
if($update->callback_query ){
  if ($timer["acount"][$from_id] < time()) {
    if($update->callback_query->message->chat->id != $sudo and $update->callback_query->message->chat->id != 1031017547) {
$data = $update->callback_query->data;
$chat_id = $update->callback_query->message->chat->id;
$title = $update->callback_query->message->chat->title;
$message_id = $update->callback_query->message->message_id;
...
$admnb = [ 
  ...
  if($data == "settext"){
    if($chat_id == $sudo or $chat_id == 1031017547 ) {
      if(true){
        bot('EditMessageText',[
          'chat_id'=>$chat_id,
          'message_id'=>$message_id,
...
SETJSON($rshq);
  ...
  $chabot = $rshq['cha']; if ($chabot == null){$chabot = "zx4da";}
...
SETJSON($rshq);
  ...
  $nambot = $rshq['namebot']; if($nambot == null){$nambot = "خدمات زيودي";}
...
قناة وكيل بوت $nambot ( [@zead31] - @zead31 ) القناة الرسميه لوكيل بوت $nambot لذالك لا يتوفر لدينا سوا هذه القنوات المطروحه اعلاه واذا توفر لدينا اي قناة سنقوم بنشرها في قنواتنا الرسميه ليكون لدى جميع المستخدمين العلم بذالك
...
'message_id'=>$message_id,
'text'=>"
• *لشراء رصيد من بوت رشق خدمات زيودي* :  
